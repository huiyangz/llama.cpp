...
struct server_routes {
  ...
  // handlers using lambda function, so that they can capture `this` without
  // `std::bind` they won't be called until ctx_http.is_ready is set to true
  ...
  server_http_context::handler_t get_model_info;
...
private:
  ...
  handle_slots_save(const server_http_req &req, int id_slot);
  std::unique_ptr<server_res_generator>
  handle_slots_restore(const server_http_req &req, int id_slot);
  std::unique_ptr<server_res_generator>
  handle_slots_erase(const server_http_req &, int id_slot);
  std::unique_ptr<server_res_generator>
  handle_embeddings_impl(const server_http_req &req,
  ...
  std::unique_ptr<server_res_generator>
  create_response(bool bypass_sleep = false);
};
