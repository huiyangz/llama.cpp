...
enum slot_state {
  SLOT_STATE_IDLE,
  SLOT_STATE_WAIT_OTHER, // after assigning a task, but waiting for parent slot
                         // to process prompt
...
private:
  ...
  bool load_model(const common_params &params) {
    ...
    if (params_base.has_speculative()) {
      ...
      vocab_dft_compatible =
          common_speculative_are_compatible(ctx, llama_init_dft->context());
  ...
  bool init() {
    ...
    queue_tasks.on_new_task(
        [this](server_task &&task) { process_single_task(std::move(task)); });
    queue_tasks.on_update_slots([this]() { update_slots(); });
    queue_tasks.on_sleeping_state(
        [this](bool sleeping) { handle_sleeping_state(sleeping); });
  ...
  construct_lora_list(const std::map<int, float> &config) {
    std::vector<common_adapter_lora_info> output =
        params_base.lora_adapters; // copy
  ...
  bool launch_slot_with_task(server_slot &slot, server_task &&task) {
    ...
    // initialize samplers
    {
      ...
      // TODO: speculative decoding requires multiple samples per batch - not
      // supported yet
      backend_sampling &= !(slot.ctx_dft && task.params.speculative.n_max > 0);

      // TODO: getting post/pre sampling logits is not yet supported with
      // backend sampling
  ...
  bool tokenize_cli_input(server_task &task) {
    ...
    try {
      ...
      if (mctx != nullptr) {
        ...
        task.tokens = std::move(
            tokenize_input_prompts(vocab, mctx, prompt, true, true)[0]);
      }
        ...
        if (n_alora_tokens) {
          ...
          for (uint64_t j = 0; j < n_alora_tokens; ++j) {
            alora_invocation_string +=
                common_token_to_piece(vocab, alora_tokens[j]);
  ...
  void update_slots() {
    ...
    for (auto &slot : slots) {
      ...
      if (n_draft_max > 0) {
        ...
        llama_tokens draft = common_speculative_gen_draft(
            slot.spec, params_spec, cached_text_tokens, slot.sampled);
        ...
        if (slot.task->params.speculative.n_min > (int)draft.size()) {
          ...
          for (size_t i = 0; i < draft.size(); i++) {
            slot.i_batch_dft.push_back(batch.n_tokens);
            common_batch_add(batch, draft[i], slot.prompt.tokens.pos_next(),
                             {slot.id}, true);
          ...
          if (slot.state == SLOT_STATE_STARTED) {
            ...
            if (!slot.can_split()) {
              if (slot.task->n_tokens() > n_ubatch) {
                send_error(slot,
                           string_format("input (%d tokens) is too large to "
                                         "process. increase the physical batch "
              ...
              if (slot.task->n_tokens() > slot.n_ctx) {
                send_error(
                    slot,
                    string_format("input (%d tokens) is larger than the max "
                                  "context size (%d tokens). skipping",
              ...
              if (slot.task->n_tokens() >= slot.n_ctx) {
                send_error(slot,
                           string_format("request (%d tokens) exceeds the "
                                         "available context size (%d "
              ...
              if (slot.task->params.cache_prompt) {
                ...
                if (!can_cache_reuse && n_cache_reuse > 0) {
                  SLT_WRN(slot,
                          "cache reuse is not supported - ignoring "
                          "n_cache_reuse = %d\n",
                          n_cache_reuse);
                }
            ...
            // this is to signal the client that the request has started
            // processing
    ...
    for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
      ...
      // technically, measuring the time here excludes the sampling time for the
      // last batch but on the other hand, we don't want to do too many system
      // calls to measure the time, so it's ok
      ...
      for (auto &slot : slots) {
        ...
        const auto ids = common_sampler_sample_and_accept_n(
            slot.smpl.get(), ctx, slot.i_batch_dft, slot.drafted);
        slot.i_batch_dft.clear();
        slot.drafted.clear();
  ...
  server_response_reader get_response_reader() {
    return server_response_reader(queue_tasks, queue_results,
...
server_context_meta server_context::get_meta() const {
  auto tool_use_src =
      common_chat_templates_source(impl->chat_templates.get(), "tool_use");

  auto bos_id = llama_vocab_bos(impl->vocab);
  auto eos_id = llama_vocab_eos(impl->vocab);
  auto bos_token_str = bos_id != LLAMA_TOKEN_NULL
                           ? common_token_to_piece(impl->ctx, bos_id, true)
                           : "";
  auto eos_token_str = eos_id != LLAMA_TOKEN_NULL
                           ? common_token_to_piece(impl->ctx, eos_id, true)
                           : "";
  ...
  return server_context_meta{
      ...
      /* chat_template          */
      common_chat_templates_source(impl->chat_templates.get()),
...
struct server_res_generator : server_http_res {
  ...
  server_res_generator(server_queue &queue_tasks,
                       server_response &queue_results, int sleep_idle_seconds,
...
std::unique_ptr<server_res_generator> server_routes::handle_completions_impl(
    const server_http_req &req, server_task_type type, const json &data,
    const std::vector<raw_buffer> &files, task_response_type res_type) {
  ...
  try {
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
      ...
      task.params = server_task::params_from_json_cmpl(ctx_server.vocab, params,
                                                       meta->slot_n_ctx, data);
  ...
  if (!stream) {
    ...
    res->next = [res_this = res.get(), res_type,
                 &req](std::string &output) -> bool {
      static auto format_error = [](task_response_type res_type,
                                    const json &res_json) {
        if (res_type == TASK_RESPONSE_TYPE_ANTHROPIC) {
      ...
      try {
        ...
        if (result->is_error()) {
          ...
          SRV_DBG("%s",
                  "error received during streaming, terminating stream\n");
...
server_routes::create_response(bool bypass_sleep) {
  return std::make_unique<server_res_generator>(
      queue_tasks, queue_results, params.sleep_idle_seconds, bypass_sleep);
}

server_routes::server_routes(const common_params &params,
                             server_context &ctx_server)
    : params(params), ctx_server(*ctx_server.impl),
...
void server_routes::init_routes() {
  ...
  // this is to ensure that the server_res_generator can handle sleeping case
  // correctly
  ...
  this->get_metrics = [this](const server_http_req &req) {
    ...
    if (!params.endpoint_metrics) {
      res->error(format_error_response("This server does not support metrics "
  ...
  this->get_slots = [this](const server_http_req &req) {
    ...
    if (!params.endpoint_slots) {
      res->error(format_error_response("This server does not support slots "
    ...
    if (!req.get_param("fail_on_no_slot").empty()) {
      if (res_task->n_idle_slots == 0) {
        res->error(
            format_error_response("no slot available", ERROR_TYPE_UNAVAILABLE));
        return res;
      }
    }
  ...
  this->post_slots = [this](const server_http_req &req) {
    ...
    if (params.slot_save_path.empty()) {
      res->error(
          format_error_response("This server does not support slots action. "
  ...
  this->post_props = [this](const server_http_req &) {
    ...
    if (!params.endpoint_props) {
      res->error(
          format_error_response("This server does not support changing global "
  ...
  this->post_infill = [this](const server_http_req &req) {
    ...
    if (data.contains("prompt") && !data.at("prompt").is_string()) {
      // prompt is optional
      res->error(format_error_response("\"prompt\" must be a string",
                                       ERROR_TYPE_INVALID_REQUEST));
    }

    if (!data.contains("input_prefix")) {
      res->error(format_error_response("\"input_prefix\" is required",
                                       ERROR_TYPE_INVALID_REQUEST));
    }

    if (!data.contains("input_suffix")) {
      res->error(format_error_response("\"input_suffix\" is required",
                                       ERROR_TYPE_INVALID_REQUEST));
    }
    ...
    if (data.contains("input_extra") && !data.at("input_extra").is_array()) {
      ...
      res->error(
          format_error_response("\"input_extra\" must be an array of "
    ...
    for (const auto &chunk : input_extra) {
      ...
      if (!chunk.contains("text") || !chunk.at("text").is_string()) {
        res->error(format_error_response("extra_context chunk must contain a "
    ...
    std::vector<server_tokens> tokenized_prompts = tokenize_input_prompts(
        ctx_server.vocab, ctx_server.mctx, prompt, false, true);
    SRV_DBG("creating infill tasks, n_prompts = %d\n",
            (int)tokenized_prompts.size());
    data["prompt"] = format_prompt_infill(
        ctx_server.vocab, data.at("input_prefix"), data.at("input_suffix"),
        data.at("input_extra"), params.n_batch, params.n_predict,
        meta->slot_n_ctx, params.spm_infill,
        tokenized_prompts[0]
            .get_text_tokens() // TODO: this could maybe be multimodal.
    );

    std::vector<raw_buffer> files; // dummy
    return handle_completions_impl(
        req, SERVER_TASK_TYPE_INFILL, data, files,
  ...
  this->post_completions = [this](const server_http_req &req) {
    ...
    return handle_completions_impl(req, SERVER_TASK_TYPE_COMPLETION, body,
                                   files, TASK_RESPONSE_TYPE_NONE);
  };
  ...
  this->post_completions_oai = [this](const server_http_req &req) {
    ...
    const json body = json::parse(req.body);
    return handle_completions_impl(req, SERVER_TASK_TYPE_COMPLETION, body,
                                   files, TASK_RESPONSE_TYPE_OAI_CMPL);
  };
    ...
    json models = {
        {"models",
         {{{"name", meta->model_name},
           ...
           {"description", ""},
           {"tags", {""}},
           {"capabilities", meta->has_mtmd ? json({"completion", "multimodal"})
                                           : json({"completion"})},
           {"parameters", ""},
           {"details",
            {{"parent_model", ""},
  ...
  this->post_rerank = [this](const server_http_req &req) {
    ...
    if (body.count("query") == 1) {
      query = body.at("query");
      if (!query.is_string()) {
        res->error(format_error_response("\"query\" must be a string",
                                         ERROR_TYPE_INVALID_REQUEST));
        return res;
      }
    } else {
      res->error(format_error_response("\"query\" must be provided",
                                       ERROR_TYPE_INVALID_REQUEST));
      return res;
    }
    ...
    auto &rd = res->rd;
    {
      ...
      for (size_t i = 0; i < documents.size(); i++) {
        auto tmp = format_prompt_rerank(ctx_server.model, ctx_server.vocab,
    ...
    // write JSON response
    json root = format_response_rerank(body, meta->model_name, responses,
                                       is_tei_format, documents, top_n);
  ...
  this->get_lora_adapters = [this](const server_http_req &req) {
    ...
    GGML_ASSERT(dynamic_cast<server_task_result_get_lora *>(result.get()) !=
                nullptr);
  ...
  this->post_lora_adapters = [this](const server_http_req &req) {
    ...
    const json body = json::parse(req.body);
    if (!body.is_array()) {
      res->error(format_error_response("Request body must be an array",
                                       ERROR_TYPE_INVALID_REQUEST));
      return res;
    }
  ...
  this->get_model_info = [this](const server_http_req &) {
    auto res = create_response(true);
    // access model from implementation;
    auto *model = ctx_server.model;

    if (!model) {
      res->error(format_error_response("Model not loaded", ERROR_TYPE_SERVER));
      return res;
    }

    auto serialize_tensor = [](const ggml_tensor *t) -> json {
      if (!t)
        return nullptr;
      json j;
      j["name"] = t->name;
      j["type"] = ggml_type_name(t->type);
      j["shape"] = json::array();
      for (int i = 0; i < GGML_MAX_DIMS; i++) {
        j["shape"].push_back(t->ne[i]);
      }
      j["size_bytes"] = ggml_nbytes(t);
      return j;
    };
    ...
    root["n_layers"] = model->layers.size();
    root["n_embd"] = model->hparams.n_embd;
    root["n_head"] = model->hparams.n_head();
    root["n_head_kv"] = model->hparams.n_head_kv();
    root["n_vocab"] = model->vocab.n_tokens();
    root["n_ctx_train"] = model->hparams.n_ctx_train;

    // Metadata for architecture-specific visualization
    char buf[256];
    if (llama_model_meta_val_str(model, "general.architecture", buf,
                                 sizeof(buf)) > 0) {
      root["metadata"]["arch"] = std::string(buf);
    } else {
      root["metadata"]["arch"] = "unknown";
    }

    std::string arch_key = root["metadata"]["arch"].get<std::string>();
    std::string act_key = arch_key + ".feed_forward.gate_activation";
    if (llama_model_meta_val_str(model, act_key.c_str(), buf, sizeof(buf)) >
        0) {
      root["metadata"]["activation"] = std::string(buf);
    } else if (llama_model_meta_val_str(model,
                                        "llama.feed_forward.gate_activation",
                                        buf, sizeof(buf)) > 0) {
      root["metadata"]["activation"] = std::string(buf);
    } else {
      root["metadata"]["activation"] = "silu"; // Default for most Llama models
    }

    if (model->hparams.use_alibi) {
      root["metadata"]["pos_embd"] = "ALiBi";
    } else if (model->hparams.rope_type != LLAMA_ROPE_TYPE_NONE) {
      root["metadata"]["pos_embd"] = "RoPE";
    } else {
      root["metadata"]["pos_embd"] = "None";
    }
    ...
    for (size_t i = 0; i < model->layers.size(); ++i) {
      ...
      json l_json;
      l_json["index"] = i;
      l_json["tensors"] = json::array();

      auto add_t = [&](const char *role, const ggml_tensor *t) {
        if (t) {
          json tj = serialize_tensor(t);
          tj["role"] = role;
          l_json["tensors"].push_back(tj);
        }
      };

      add_t("attn_norm", layer.attn_norm);
      add_t("attn_norm_b", layer.attn_norm_b);
      add_t("attn_q", layer.wq);
      add_t("attn_k", layer.wk);
      add_t("attn_v", layer.wv);
      add_t("attn_o", layer.wo);
      add_t("attn_q_b", layer.bq);
      add_t("attn_k_b", layer.bk);
      add_t("attn_v_b", layer.bv);
      add_t("attn_o_b", layer.bo);

      add_t("ffn_norm", layer.ffn_norm);
      add_t("ffn_norm_b", layer.ffn_norm_b);
      add_t("ffn_gate", layer.ffn_gate);
      add_t("ffn_down", layer.ffn_down);
      add_t("ffn_up", layer.ffn_up);
      add_t("ffn_gate_b", layer.ffn_gate_b);
      add_t("ffn_down_b", layer.ffn_down_b);
      add_t("ffn_up_b", layer.ffn_up_b);

      // MoE
      add_t("ffn_gate_inp", layer.ffn_gate_inp);
      add_t("ffn_gate_exps", layer.ffn_gate_exps);
      add_t("ffn_down_exps", layer.ffn_down_exps);
      add_t("ffn_up_exps", layer.ffn_up_exps);

      layers.push_back(l_json);
    }
    root["layers"] = layers;
    ...
    auto add_g = [&](const char *role, const ggml_tensor *t) {
      if (t) {
        json tj = serialize_tensor(t);
        tj["role"] = role;
        globals.push_back(tj);
      }
    };
    add_g("tok_embd", model->tok_embd);
    add_g("type_embd", model->type_embd);
    add_g("pos_embd", model->pos_embd);
    add_g("tok_norm", model->tok_norm);
    add_g("output_norm", model->output_norm);
    add_g("output", model->output);
...
std::unique_ptr<server_res_generator>
server_routes::handle_slots_save(const server_http_req &req, int id_slot) {
  auto res = create_response();
  const json request_data = json::parse(req.body);
  std::string filename = request_data.at("filename");
  if (!fs_validate_filename(filename)) {
    res->error(
        format_error_response("Invalid filename", ERROR_TYPE_INVALID_REQUEST));
    return res;
  }
...
std::unique_ptr<server_res_generator>
server_routes::handle_slots_restore(const server_http_req &req, int id_slot) {
  auto res = create_response();
  const json request_data = json::parse(req.body);
  std::string filename = request_data.at("filename");
  if (!fs_validate_filename(filename)) {
  ...
  GGML_ASSERT(dynamic_cast<server_task_result_slot_save_load *>(result.get()) !=
              nullptr);
  res->ok(result->to_json());
  return res;
}
...
std::unique_ptr<server_res_generator>
server_routes::handle_slots_erase(const server_http_req &req, int id_slot) {
  auto res = create_response();
  auto &rd = res->rd;
  {
...
std::unique_ptr<server_res_generator>
server_routes::handle_embeddings_impl(const server_http_req &req,
  ...
  if (res_type != TASK_RESPONSE_TYPE_NONE &&
      meta->pooling_type == LLAMA_POOLING_TYPE_NONE) {
    res->error(
  ...
  if (body.count("input") != 0) {
    ...
    res->error(
        format_error_response("\"input\" or \"content\" must be provided",
                              ERROR_TYPE_INVALID_REQUEST));
    return res;
  }
  ...
  if (body.count("encoding_format") != 0) {
    ...
    if (format == "base64") {
      ...
      res->error(format_error_response("The format to return the embeddings "
  ...
  for (const auto &tokens : tokenized_prompts) {
    ...
    if (tokens.empty()) {
      res->error(format_error_response("Input content cannot be empty",
                                       ERROR_TYPE_INVALID_REQUEST));
      return res;
    }
  }
  ...
  json root = res_type == TASK_RESPONSE_TYPE_OAI_EMBD
                  ? format_embeddings_response_oaicompat(body, meta->model_name,
                                                         responses, use_base64)
                  : json(responses);
  res->ok(root);
  return res;
}
